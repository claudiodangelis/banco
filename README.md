...
Presents a fuzzy-select menu: provider → module → item → page (if the item has more than one page). The selected URL is opened via `$BROWSER`, `xdg-open`, `open`, or `cmd /c start`, tried in that order.

**Browseable items per provider:**
